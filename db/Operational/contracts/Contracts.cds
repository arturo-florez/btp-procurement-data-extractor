...
entity Contracts_OP : managed, types.customFields {
    ...
    key UniqueName                       : String(50);
        ...
        ForecastedSpend_Items            : Composition of many Contracts_ForcastedSpendItem_OP
                                               on ForecastedSpend_Items.Contract = $self;

        ParentAgreement                  : types.parentAgreement;

        SubAgreements                    : Composition of many Contracts_SubAgreements_OP
                                               on SubAgreements.Contract = $self;
        LineItems                        : Composition of many Contracts_LineItem_OP
                                               on LineItems.Contracts = $self;
}
...
entity Contracts_LineItem_OP : cuid {
    key Contracts                       : Association to Contracts_OP;
        ...
        PricingTerms_TieredPricingSteps : Composition of many Contracts_LineItem_TieredPricingSteps_OP
                                              on PricingTerms_TieredPricingSteps.LineItem = $self;

        SplitAccountings                : Composition of many Contracts_LineItem_SplitAccountings_OP
